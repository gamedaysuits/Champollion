...
description: 'The agent front door: what Champollion adds to a language model, which surface to use for which job, the machine-readable endpoints, and the license and sovereignty rules that bind automated use.'
---

<!--
  AGENT-FACING FRONT DOOR. This page is the SOURCE for the raw markdown
  artifact served at /for-agents.md (the llms.txt-style convention: the same
  page, fetchable as plain markdown by an agent with no HTML parser).

    src/pages/for-agents.md   (this file — the SSOT, rendered at /for-agents)
      → scripts/build-for-agents-md.mjs
      → static/for-agents.md  (derived, committed; links absolutized)

  After editing this file, regenerate the artifact:

      cd cli/website && node scripts/build-for-agents-md.mjs

  The pre-push gate (scripts/champollion_sync_gate.sh) hard-blocks on drift.

  HOUSE RULES THAT APPLY HERE (see CLAUDE.md):
  - No hand-typed exact counts. Language count stays a FLOOR ("7,900+" —
    same deliberate rot-proofing as the middleware gate page). Tool counts,
    provider counts, dataset counts: link to the page that carries them
    under a parity gate instead of repeating the number.
  - "sovereignty-aspirant", never sovereignty-validated.
  - Prize pools are conditional ("when active"), never asserted as running.
-->
...
Protocol](/docs/network/getting-started/mcp-server), designed so that an
...
- **An agent with a translation job** — an app's locale files, Markdown
  ...
  the [Agent Guide: Using champollion](/docs/guides/agent-guide).
- **An agent building or benchmarking a translation method** — including the
  ...
  Benchmarking on the Network](/docs/network/getting-started/agent-guide).
- **An agent that needs facts about a language** — classification, speaker
  ...
  [Atlas](/docs/atlas), where every value is cited and disagreeing sources
  are shown side by side rather than resolved for you.
- **A developer wiring an agent up** — install the
  [MCP server](/docs/network/getting-started/mcp-server) and hand your agent
  the tools.
...
|---|---|---|
| [MCP server](/docs/network/getting-started/mcp-server) | `npx -y champollion-mcp-server` | Acting stepwise: translate, query language cards, browse the benchmark queue, run evaluations, train models |
| [champollion CLI](/docs/guides/agent-guide) | `npx champollion sync` | Batch translation of a repo's locale files or Markdown content |
| [mt-eval harness](/docs/network/getting-started/agent-guide) | `pip install mt-eval-harness` | Benchmarking a method, reproducing published runs |
...
[MCP server page](/docs/network/getting-started/mcp-server).
...
- **Transmission is consent-first.** Restricted-license corpora
  ...
  table](/docs/network/leaderboard/datasets).
- **Language data is treated with the care of biodata.** The project is
  ...
  [Data Sovereignty](/docs/learn/data-sovereignty) — an attempt to enact
  Indigenous data-sovereignty principles in code, open to critique, not a validation claim.
- **Trust the limits.** [Honest
  Limitations](/docs/network/honest-limitations) states what this project
...
Specification](/docs/network/specifications/prizes).

Start with the [Network Agent
Guide](/docs/network/getting-started/agent-guide), or fetch the
[queue preview](https://champollion.dev/queue-preview.json) to see which
language pairs are waiting for a method.

## Learn more

- [What Champollion Is](/docs/what-is-champollion) — the human front door
- [The Arena](/docs/arena) — where measurement happens: submission door, run-card ledger, contest machinery
- [The Atlas](/docs/atlas) — the language index: what a card may assert, and what it is forbidden to assert
- [CLI Reference](/docs/reference/cli) — every command and option
- [Eval Harness specification](/docs/network/specifications/harness) — the scoring contract
