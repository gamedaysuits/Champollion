...
> **목적.** 이 문서는 LYSS 평가 지표를 검증하기 위해 이중언어(Cree-English) 구사자에게 정확히 무엇이 필요한지 정의해요. 이러한 검증이 없다면, 자동화된 점수는 입증된 품질 측정값이 아니라 엔지니어링 추정치에 불과해요. 이는 프로젝트에서 가장 중요한 공백이에요.
>
> **대상 독자.** 커뮤니티 파트너, 잠재적 협력자, 보조금 심사위원 및 프로젝트 팀.
...
1. 436개 항목으로 구성된 EdTeKLA 개발 코퍼스(`textbook_dev.json`)에서 FST 분석기를 실행하고 거부된 모든 단어를 수집해요.
2. 거부된 단어 중 최대 100개를 문장 문맥과 함께 화자에게 제시해요.
3. 각 단어에 대해 화자는 다음과 같이 답변해요:
   - **이것은 유효한 Cree 단어인가요?** 예 / 아니요 / 확실하지 않음
   - **그렇다면, 어떤 종류인가요?** 기존 단어 / 외래어 / 이름 / 방언 형태 / 신조어 / 기타
   - **참고 사항** (선택 사항)
...
모든 화자의 기여는 프로젝트의 주권 지향(sovereignty-aspirant) 데이터 정책의 적용을 받아요:
...
화자 검증은 더 큰 파이프라인의 한 단계예요:
[코퍼스 파트너십](/docs/network/specifications/corpus-partnership)
워크플로우는 이러한 작업이 평가 코퍼스를 구축하는 과정에 어떻게 들어맞는지 보여주며,
[화자 지급 방식](/docs/network/perspectives/how-speakers-get-paid)은
위의 요율과 조건을 쉬운 말로 설명한 버전이에요.
