...
> **目的。** このドキュメントは、LYSS評価指標を検証するために、Cree語と英語のバイリンガル話者に何を求めているかを正確に定義するものです。この検証がなければ、私たちの自動スコアはエンジニアリング上の推定値に過ぎず、証明された品質測定値にはなりません。これは、プロジェクトにおいて最も重要なギャップです。
>
> **対象読者。** コミュニティパートナー、潜在的な協力者、助成金審査員、およびプロジェクトチーム。
...
1. 436エントリのEdTeKLA開発コーパス（`textbook_dev.json`）に対してFSTアナライザーを実行し、拒否されたすべての単語を収集します。
2. 拒否された単語のうち最大100語を、文脈とともに話者に提示します。
3. 各単語について、話者は以下に回答します。
   - **これは有効なCree語の単語ですか？** はい / いいえ / わからない
   - **「はい」の場合、どの種類ですか？** 既存の単語 / 借用語 / 名前 / 方言 / 新語 / その他
   - **備考** （任意）
...
話者のすべての貢献は、プロジェクトの主権の実現を目指す（sovereignty-aspirant）データポリシーによって管理されます。
