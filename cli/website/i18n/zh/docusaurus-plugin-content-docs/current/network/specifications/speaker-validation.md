...
> **目的。** 本文档确切定义了我们需要 Cree-英语双语使用者提供哪些内容，以验证 LYSS 评估指标。如果没有此项验证，我们的自动化评分就只是工程估算，而非经过证实的质量衡量标准。这是本项目中最重要的一项空白。
>
> **受众。** 社区合作伙伴、潜在合作者、资助评审员以及项目团队。
...
1. 我们在包含 436 个条目的 EdTeKLA 开发语料库 (`textbook_dev.json`) 上运行 FST 分析器，并收集其拒绝的每一个词。
2. 我们向说话人展示最多 100 个被拒绝的词及其句子上下文。
3. 对于每个词，说话人需回答：
   - **这是一个有效的 Cree 词汇吗？** 是 / 否 / 不确定
   - **如果是，属于哪种类型？** 既有词汇 / 外来词 / 专有名词 / 方言形式 / 新造词 / 其他
   - **备注**（可选）
...
说话人验证是更大处理流程中的一个阶段：
[语料库合作伙伴关系](/docs/network/specifications/corpus-partnership)
工作流说明了这些任务如何融入评估语料库的构建中，而
[说话人如何获得报酬](/docs/network/perspectives/how-speakers-get-paid)
则是上述费率和条款的通俗语言版本。
