...
> **执行摘要。** 本文档定义了 Champollion MT 评估生态系统的评估协议：语料库格式（§2）、运行卡 schema（§3）、基准测试协议（§6）、人工验证要求（§7）、主权机制（§8）、排行榜与提交模型（§9）、成本框架（§10）以及对新语言的扩展性（§11）。有关指标定义、综合评分权重、质量层级阈值以及成本/速度指标公式，请参阅 `SCORING_SPEC.md` —— 这是所有评分逻辑的唯一事实来源。本文档引用了 SCORING_SPEC 中的这些详细信息，而不是重复说明。
...
没有自动化评分，无论多高，都无法替代流利使用者阅读输出并确认其正确、自然且文化上适当。§5 中定义的质量等级是自动化复合评分上的启发式标签 — 对于跟踪进度很有用，但单独永远不够。
...
> **📎 = 推荐。** 工具优雅地通过默认值处理缺失的可选字段。第三方语料库每个条目只需提供 `id`、`source` 和 `reference`。
...
当运行卡发布到排行榜时（通过 `mt-eval publish`），它还包含一个 `method_config` 块，其中包含规范的 8 字段 MethodConfig（`model`、`temperature`、`batchSize`、`register`、`coachingFile`、`coachingPrompt`、`promptContext`、`qualityTier` — 全部采用驼峰式命名）。这样可以实现零重构导入：`champollion leaderboard --install` 直接读取 `method_config` 并将其写入插件清单。上面的遥测字段（§3.2）记录了测试框架观察到的内容；`method_config` 记录了开发者的意图。
:::
...
| `scores.equivalent_matches` | number | 匹配可接受变体的条目 |
...
| `scores.morphological_accuracy` | number | 0.0–1.0，FST 派生（词根匹配），如果没有 FST / 没有词根匹配的词则为 `null`。咨询直到激活 — 见评分规范 §2.2 |
| `scores.morph_coverage` | number | 0.0–1.0，可分析的预测词中词根匹配到参考的分数（披露 `morphological_accuracy` 有多稀疏） |
...
| `scores.length_ratio` | number | avg(len(predicted)/len(reference))，理想 = 1.0 |
...
| `scores.by_domain` | object | ✅ 已实现 — 按领域分解的评分（§2.7）。启用领域过滤的排行榜排名。由 tester.py 计算并通过 publish.py 传递。 |
...
| 指标 | 状态 | 测量内容 | 范围 |
|--------|--------|-----------------|-------|
| **chrF++** | ✅ 已实现 | 字符 n-gram F 分数。在字符级别运作，对于词长且高度屈折的形态学丰富语言比词级指标（BLEU）更稳健。由 sacrebleu 计算。 | 0–100（原生规模）。在复合中使用时除以 100。 |
| **FST 接受率** | ✅ 已实现 | 形态学分析器（GiellaLT HFST）接受的预测词的分数作为目标语言中的有效形式。FST 接受的词是真实的、结构上有效的词 — 不是幻觉。 | 0.0–1.0 |
...
> **为什么不是 BLEU？** BLEU 在词级别运作并惩罚形态学变化。对于多综合语言，单个词可以是整个从句 — BLEU 会将轻微的屈折差异视为完全错误。chrF++ 通过在字符级别运作来更好地处理这个问题。BLEU 从两个权重表中排除。见 SCORING_SPEC 附录 A 了解完整理由。
...
质量等级是自动化复合评分上的启发式标签。它们根据在每个级别对输出的人工审查，描述评分在实践中往往意味着什么。**它们不是经过验证的质量判断** — 只有人工审查（§6）可以确认实际可用性。

**等级阈值和描述在 `SCORING_SPEC.md` §5 中定义。** 等级为：基线（0.00–0.30）、新兴（0.30–0.50）、功能性（0.50–0.70）、可部署（0.70–0.85）和流利（0.85–1.00）。

> [!IMPORTANT]
> **自动化等级是暂定的。** 这些标签是供审查的提名，而不是质量声明。在自动化指标上达到"可部署"的方法是社区评估的候选 — 不是要发布的产品。只有人工审查（§7）可以确认实际可用性。等级边界可能因语言而异。
...
没有方法可以从**功能性**推进到**可部署**等级，除非人工验证确认双语使用者同意输出是可用的。这不是形式 — 这是重点。自动化指标的存在是为了减少需要人工审查的输出量。它们无法替代它。
...
3. **承诺评估可用性** — 基准必须保持可评估
4. **发布参与条款** — 开发者同意的明确文档
5. **在公认的数据主权原则下运作** — 语言数据由社区拥有和控制、CARE 或等效

### 8.5 践行数据主权与 CARE 原则

| 原则 | 实现 |
|-----------|---------------|
| **所有权** | 语言数据属于社区。治理组织控制评估基础设施。 |
| **控制权** | 治理组织通过沙盒执行来控制评估。他们决定谁可以提交以及提交的条款。 |
| **访问权** | 社区可以无限制地访问他们自己的数据、结果以及基于此开发的方法。 |
| **占有权** | 测试集绝不会离开治理基础设施。静态加密作为备份。 |
| **集体利益** (CARE) | 所有权转移确保方法造福社区，社区保留该方法及其产生的所有收益 —— 平台不抽取任何分成。 |
| **控制权** (CARE) | 沙盒执行是其技术实现。 |
| **责任** (CARE) | 开发者通过参与条款承担责任。 |
| **伦理** (CARE) | 社区权利高于研究人员的便利。 |
...
- 将推理请求代理至**固定模型的显式白名单** —— 即记录在方法清单（manifest）和运行卡中的模型标识符；
- **记录每一次请求和响应**，保存在仅追加（append-only）、哈希链式的审计日志中，以便在发布分数之前审查网关流量是否存在数据外泄企图；
- 是*唯一*的网络路径 —— 没有通用出口，没有 DNS，也没有其他端点。
...
**状态。** 网络隔离的方法执行**沙盒已实现**，用于组织者举办的竞赛（于 2026-07-08 发布；有关已构建和未构建的具体内容，请参阅 [诚实的局限性](/docs/network/honest-limitations)）。**LLM 网关已完成规范设计，但尚未构建。** 在网关投入运行之前，只有 S 类和 O 类方法能够生成黄金标准分数；A1 类方法原则上仍有资格获奖（请参阅 [奖金规范 §1.6](/docs/network/specifications/prizes)），但尚无法针对秘密片段进行评估。在权利持有人授予许可之前，A2 类依赖项根本无法进入沙盒 —— 在出现任何网络问题之前，必须先允许该制品在沙盒中*存在*。
...
| 层级 | 含义 | 如何达成 |
|------|---------|--------------|
| **自我基准测试** | 开发者运行了测试工具（harness）并提交了运行卡 | 针对 `development` 片段提交 PR 或使用 `--publish` 标志 |
| **Champollion 验证** | 维护者独立复现了结果 | 将方法作为可安装插件提交；维护者重新运行 |
| **社区验证** | 根据社区自身协议获得资格的目标语言双语使用者，审查了输出的分层样本（≥30 个条目，≥2 名审查者），且 ≥70% 达到了社区的标准。仅通过社区自身的测试授予；通过抽查降级的机制是对称的 | 将方法代码提交给治理组织（§8.2）；他们在 `gold_standard` 上运行该代码，且输出通过人工验证（§7） |
...
| 片段 | 提交路径 | 验证 | 是否需要方法代码？ |
|---------|----------------|-------------|----------------------|
| `development` | 自助服务：运行测试工具，通过 PR 或 API 提交运行卡 | 自我基准测试 | 否 —— 您保留自己的代码 |
| `development` | 维护者重新运行：将方法作为插件提交 | Champollion 验证 | 是 —— 方法必须是可安装的 |
| `gold_standard` | 将方法提交给治理组织；他们在沙盒中运行 | 社区验证 | 是 —— 方法被提交并由其保管 |
