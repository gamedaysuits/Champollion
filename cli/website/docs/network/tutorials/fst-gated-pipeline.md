...
description: "Build a decomposition pipeline with morphological validation and benchmark it on the Network leaderboard."
related:
...
:::info[Prerequisites]
- A running FST binary (e.g., the [GiellaLT/ALTLab Plains Cree analyzer](https://github.com/giellalt/lang-crk) — distributed via GiellaLT's nightly channel, not GitHub releases)
...
Or use the CLI to compare against the built-in baseline:
...
See [SCORING_SPEC §5](/docs/network/specifications/scoring#5-quality-tiers) for the full tier definitions.
:::
...
The composite score is a weighted average of available metrics, with weights defined in [SCORING_SPEC §4](/docs/network/specifications/scoring#4-composite-score). When a metric isn't available, its weight is redistributed proportionally across the rest.
...
- **[Eval Harness](/docs/network/specifications/harness)** — how to run the harness and interpret output
- **[Method Interface](/docs/network/specifications/methods)** — the `TranslationMethod` protocol specification
- **[Leaderboard Rules](/docs/network/leaderboard/rules)** — submission criteria and anti-gaming policies
- **[Support a Low-Resource Language](/docs/network/community/low-resource-languages)** — the broader context and community data-sovereignty principles
- **[ALTLab](https://altlab.ualberta.ca/)** — the Alberta Language Technology Lab (Plains Cree FST)
